...
export default function SignUpUser() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        nome: formData.nome,
        cpf: formData.cpf.replace(/\D/g, ""), // Limpa CPF
      ...
      <div className="w-full max-w-2xl bg-white p-8 md:p-10 rounded-[2rem] shadow-2xl shadow-[#253528]/5 border border-[#8BA889]/10 relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="flex flex-col items-center text-center mb-8">
            ...
            <div className="absolute left-8 top-8 hidden md:block">
                <Button variant="ghost" onClick={() => router.back()} className="text-[#8BA889] hover:text-[#49654E]">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                    <div className="space-y-2">
                        <Label htmlFor="nome" className="text-xs font-bold text-[#253528]/70 uppercase ml-1">Nome Completo</Label>
                        <div className="relative group">
                            <User className="absolute left-4 top-3.5 h-4 w-4 text-[#8BA889] group-focus-within:text-[#49654E] transition-colors" />
                            <Input id="nome" placeholder="Ex: Ana Silva" required value={formData.nome} onChange={handleChange} 
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="cpf" className="text-xs font-bold text-[#253528]/70 uppercase ml-1">CPF</Label>
                        <div className="relative group">
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="email" className="text-xs font-bold text-[#253528]/70 uppercase ml-1">Email</Label>
                        <div className="relative group">
                            <Mail className="absolute left-4 top-3.5 h-4 w-4 text-[#8BA889] group-focus-within:text-[#49654E] transition-colors" />
                            <Input id="email" type="email" placeholder="cliente@email.com" required value={formData.email} onChange={handleChange} 
                                className="pl-10 bg-[#F9FBF9] border-[#8BA889]/20 focus:border-[#49654E] h-11 rounded-xl"
                            />
                        </div>
                    </div>
                ...
                <div className="space-y-4">
                     <div className="space-y-2">
                        <Label htmlFor="endereco" className="text-xs font-bold text-[#253528]/70 uppercase ml-1">Endereço</Label>
                        <div className="relative group">
                            <MapPin className="absolute left-4 top-3.5 h-4 w-4 text-[#8BA889] group-focus-within:text-[#49654E] transition-colors" />
                            <Input id="endereco" placeholder="Rua das Flores, 123" required value={formData.endereco} onChange={handleChange} 
                                className="pl-10 bg-[#F9FBF9] border-[#8BA889]/20 focus:border-[#49654E] h-11 rounded-xl"
                            />
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="senha" className="text-xs font-bold text-[#253528]/70 uppercase ml-1">Senha Provisória</Label>
                        <div className="relative group">
                            <KeyRound className="absolute left-4 top-3.5 h-4 w-4 text-[#8BA889] group-focus-within:text-[#49654E] transition-colors" />
                            <Input id="senha" type="password" placeholder="••••••••" required value={formData.senha} onChange={handleChange} 
                                className="pl-10 bg-[#F9FBF9] border-[#8BA889]/20 focus:border-[#49654E] h-11 rounded-xl"
                            />
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        <Label htmlFor="confirmarSenha" className="text-xs font-bold text-[#253528]/70 uppercase ml-1">Confirmar Senha</Label>
                        <div className="relative group">
                            <KeyRound className="absolute left-4 top-3.5 h-4 w-4 text-[#8BA889] group-focus-within:text-[#49654E] transition-colors" />
            ...
            <Button type="submit" disabled={loading} className="w-full h-12 rounded-xl bg-[#49654E] hover:bg-[#3d5441] text-white font-bold shadow-lg shadow-[#49654E]/20 mt-6 transition-all">
                {loading ? <><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Salvando...</> : "Finalizar Cadastro"}
            </Button>
        </form>
      </div>
