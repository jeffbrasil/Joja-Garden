...
function ChangePasswordContent() {
  ...
  const handleSalvar = async (e: React.FormEvent) => {
    ...
    if (!idParam) {
      toast.error("Erro de Identificação", { description: "ID do administrador não encontrado." });
      return;
    }

    if (novaSenha !== confirmarSenha) {
      toast.error("Validação Falhou", { description: "As novas senhas não coincidem." });
      return;
    }

    if (novaSenha.length < 6) {
      toast.warning("Senha Fraca", { description: "A nova senha deve ter pelo menos 6 caracteres." });
      return;
    }
    ...
    try {
      ...
      if (error.response?.status === 400 || error.response?.status === 401) {
          toast.error("Não autorizado", { description: "A senha atual digitada está incorreta." });
      } else {
          toast.error("Erro no Sistema", { description: "Tente novamente mais tarde." });
      }
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="w-full max-w-lg bg-white rounded-[2.5rem] shadow-2xl shadow-[#253528]/10 border border-[#8BA889]/20 overflow-hidden">
          ...
          <div className="p-8 md:p-10 pb-6 text-center">
             ...
             <p className="text-[#8BA889] mt-2 text-sm">
               Você está alterando as credenciais de:<br/>
               <span className="text-[#49654E] font-bold text-base">{nomeParam}</span> <span className="text-xs opacity-70">(ID: {idParam})</span>
             </p>
          </div>
          ...
          <div className="px-8 md:px-10 pb-10">
            ...
            <div className="mb-6 bg-amber-50 border border-amber-100 rounded-2xl p-4 flex gap-3 items-start">
              ...
              <p className="text-xs text-amber-800 leading-relaxed">
                Por segurança, é necessário informar a <strong>senha atual</strong> deste administrador para validar a troca.
              </p>
            </div>
            ...
            <form onSubmit={handleSalvar} className="space-y-5">
              ...
              <div className="space-y-2">
                <Label className="text-xs font-bold text-[#253528]/70 uppercase ml-1">Senha Atual</Label>
                <div className="relative group">
                  ...
                  />
                  <button type="button" onClick={() => setShowCurrent(!showCurrent)} className="absolute right-3 top-3.5 text-[#8BA889] hover:text-[#49654E]">
              ...
              {/* Nova Senha */}
              <div className="space-y-2">
                <Label className="text-xs font-bold text-[#253528]/70 uppercase ml-1">Nova Senha</Label>
                <div className="relative group">
                  ...
                  <Input
                    ...
                    className="pl-12 pr-10 h-12 rounded-xl border-[#8BA889]/20 bg-[#F9FBF9] focus:border-[#49654E] focus:ring-[#49654E]/20 transition-all"
                  />
                   <button type="button" onClick={() => setShowNew(!showNew)} className="absolute right-3 top-3.5 text-[#8BA889] hover:text-[#49654E]">
              ...
              {/* Confirmar Senha */}
              <div className="space-y-2">
                <Label className="text-xs font-bold text-[#253528]/70 uppercase ml-1">Confirmar Nova Senha</Label>
                <div className="relative group">
                   <Key className="absolute left-4 top-3.5 h-5 w-5 text-[#8BA889] group-focus-within:text-[#49654E] transition-colors" />
                  <Input
                  ...
                  />
                  <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="absolute right-3 top-3.5 text-[#8BA889] hover:text-[#49654E]">
                    {showConfirm ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                  </button>
                </div>
                {confirmarSenha && novaSenha !== confirmarSenha && (
                  <p className="text-xs text-red-500 font-medium ml-1 animate-in slide-in-from-top-1">As senhas não coincidem.</p>
                )}
              </div>
