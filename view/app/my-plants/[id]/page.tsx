    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        const [plantRes, actionsRes, galleryRes] = await Promise.all([
          api.get(`/planta/${plantId}`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
          api.get(`/acao/${plantId}/acoes`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
          api.get(`/imagem/${plantId}/galeria`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
  ...
  const handleRegisterAction = async () => {
    ...
    try {
      ...
      showFeedback(
        "success",
        "Cuidado Registrado!",
        `Sua ação de ${payload.tipo} foi salva com sucesso.`,
      );
    } catch (error) {
      showFeedback(
        "error",
  ...
  const handleAddImage = async () => {
    ...
    try {
      ...
      showFeedback(
        "success",
        "Foto Adicionada!",
        "Sua galeria está ficando linda.",
      );
    } catch (error) {
      showFeedback(
        "error",
        "Erro ao salvar",
        "Verifique a URL da imagem e tente novamente.",
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const handleDeleteImage = async () => {
    ...
    try {
      ...
      showFeedback(
        "success",
        "Imagem Removida",
        "A foto foi excluída da galeria.",
      );
    } catch (error) {
      showFeedback(
        "error",
  ...
  return (
    <div className="min-h-screen bg-quinquenary pb-24">
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-white/60 text-sm flex items-center">
                  <MapPin className="w-3 h-3 mr-1" /> Jardim #1{" "}
                  {/* Exemplo estático ou buscar do jardim */}
                </span>
              </div>
              ...
              <p className="text-white/80 text-lg font-light italic flex items-center gap-2">
                ...
                {plant.catalogo.nome_cientifico}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-2 rounded-3xl shadow-xl shadow-primary/10 inline-block w-full md:w-auto">
          <div className="flex flex-col md:flex-row gap-6 items-center md:items-start">
            ...
            <div className="flex-1 p-2 md:py-4 md:pr-6 w-full">
              <h3 className="text-tertiary font-medium mb-4 text-sm uppercase tracking-wider">
                Cuidados Básicos
              </h3>
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center p-3 bg-quinquenary rounded-2xl">
                  ...
                  <span className="font-bold text-primary text-sm">
                    {plant.catalogo.periodicidade_rega}d
                  </span>
                </div>
                <div className="text-center p-3 bg-quinquenary rounded-2xl">
                  ...
                  <span className="font-bold text-primary text-sm">
                    {plant.catalogo.periodicidade_poda}d
                  </span>
                </div>
                <div className="text-center p-3 bg-quinquenary rounded-2xl">
                  ...
                  <span className="font-bold text-primary text-sm">
                    {plant.catalogo.periodicidade_adubo}d
                  </span>
                </div>
              </div>
            </div>
        ...
        <Tabs defaultValue="overview" className="w-full space-y-8">
          ...
          <div className="overflow-x-auto pb-2">
            <TabsList className="bg-white shadow-sm border border-tertiary/10 p-1.5 rounded-2xl h-auto inline-flex min-w-full md:min-w-0">
              <TabsTrigger
                value="overview"
                className="gap-2 px-6 py-3 rounded-xl data-[state=active]:bg-primary data-[state=active]:text-white text-tertiary font-medium transition-all hover:text-primary"
              >
              ...
              <Card className="lg:col-span-2 border-none shadow-md rounded-3xl overflow-hidden">
                <CardHeader className="bg-quinquenary pb-2">
                  <CardTitle className="text-xl text-primary font-bold flex items-center gap-2">
                    <div className="p-2 bg-quaternary rounded-full">
                ...
                <CardContent className="space-y-6 pt-4">
                  <div>
                    <h4 className="font-semibold text-primary/80 mb-2 text-sm uppercase tracking-wide">
                      Descrição
                    </h4>
                    <p className="text-secondary leading-relaxed text-lg font-light">
                      {plant.catalogo.descricao}
                    </p>
                  </div>
                  <div>
                    <h4 className="font-semibold text-primary/80 mb-2 text-sm uppercase tracking-wide">
              ...
              <Card className="border-none shadow-md h-fit rounded-3xl bg-white">
                ...
                <CardContent className="space-y-0">
                  ...
                  <div className="flex justify-between py-4">
                    ...
                    <span className="font-mono text-secondary bg-quinquenary px-2 py-1 rounded-lg">
                      #{plant.id}
                    </span>
                  </div>
                </CardContent>
          ...
          <TabsContent
            value="diary"
            className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4"
          >
                ...
                <DialogContent className="bg-white border-none sm:rounded-2xl shadow-2xl">
                  ...
                  <div className="space-y-5 py-4">
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(val) =>
                          setNewAction({ ...newAction, tipo: val })
                        }
                      >
                  ...
                  <DialogFooter>
                    <Button
                      ...
                      className="text-tertiary rounded-xl"
                    >
                      Cancelar
                    </Button>
                    <Button
                    ...
                    <Card className="w-full hover:shadow-lg transition-all duration-300 bg-white border-none shadow-sm rounded-2xl">
                      <CardContent className="p-6">
                        <div className="flex justify-between items-start mb-3">
                          <Badge
                            variant="outline"
                            className="uppercase font-bold tracking-wider bg-quinquenary text-secondary border-none px-3 py-1"
                          >
                            {action.tipo}
                          </Badge>
                          <div className="flex items-center text-xs font-medium text-tertiary gap-1 bg-quinquenary/50 px-3 py-1 rounded-full">
                            <Clock size={12} />
                            {new Date(action.data_hora).toLocaleDateString(
                              "pt-BR",
                            )}{" "}
                            às{" "}
                            {new Date(action.data_hora).toLocaleTimeString(
                              "pt-BR",
              ...
              <Dialog open={isGalleryOpen} onOpenChange={setIsGalleryOpen}>
                ...
                <DialogContent className="bg-white border-none sm:rounded-2xl shadow-2xl">
                  <DialogHeader>
                    ...
                    <DialogDescription className="text-tertiary">
                      Adicione o link da imagem para acompanhar o crescimento.
                    </DialogDescription>
                  </DialogHeader>
                  <div className="space-y-4 py-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setNewImage({ ...newImage, titulo: e.target.value })
                        }
                      />
                    </div>
                    <div className="space-y-2">
            ...
            {gallery.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {gallery.map((img) => (
                  <div
                    key={img.id}
                    className="group relative aspect-square bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300"
                  >
                    <img
                      src={img.url}
                      alt={img.titulo}
      ...
      <AlertDialog
        ...
        onOpenChange={(open) =>
          setFeedback((prev) => ({ ...prev, isOpen: open }))
        }
      >
