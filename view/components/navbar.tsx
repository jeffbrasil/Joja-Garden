...
export default function Navbar() {
  ...
  return (
    // CORREÇÃO AQUI: Removi 'relative'. Mantive apenas 'fixed' e as outras classes visuais.
    <nav className="fixed top-0 left-0 w-full h-16 bg-quaternary/95 backdrop-blur-sm shadow-sm shadow-tertiary/20 z-50 px-6 md:px-12 flex items-center justify-between font-poppins transition-all">
      ...
      <Link
        href="/"
        // O 'absolute' funciona aqui porque o pai é 'fixed'
        className={`flex items-center gap-2 group cursor-pointer ${!user ? 'absolute left-1/2 -translate-x-1/2' : ''}`}
        onClick={closeMenu}
      >
      ...
      {/* --- MENU HAMBÚRGUER --- */}
      {/* ml-auto empurra para a direita, garantindo alinhamento correto */}
