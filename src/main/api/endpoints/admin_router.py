...
@router.post("/criar_conta", response_model=AdminResponse,status_code=status.HTTP_201_CREATED)
def create_admin(admin_in: AdminCreate, session = Depends(get_db)):
    
    user_existente = session.query(Super_usuario).filter(Super_usuario.cpf==admin_in.cpf).first()
    if user_existente:
        raise HTTPException(
                status_code=400,
